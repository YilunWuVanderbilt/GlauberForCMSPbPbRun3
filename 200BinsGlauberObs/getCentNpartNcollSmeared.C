...
void getCentNpartNcollSmeared(){
  ...
  TFile *infile = new TFile("./tables_Glauber2024A_ForHydjetLHC5036Responsenbins200x_d20240901_Smearing975Eff.root");
...
*/
  ...
  ofstream outFileTxt;
  outFileTxt.open("./NpartNcollTAA200bins.txt");
  for(int i=0; i<nBin; i++){
    //nt->Draw("Npart>>+hNpart",Form("HFbyNpart>=%f && HFbyNpart < %f", bValues[i], bValues[i+1]));
    outFileTxt<<"i = "<<i<<", <npart>= "<<hNpartN[i]->GetMean()<<", <ncoll>= "<<hNcollN[i]->GetMean()<<", <TAA>= "<<hNcollN[i]->GetMean()/67.6<<endl;
  ...
  TFile *ofile = new TFile("default_glauber_smeared_2024hf_975eff.root","recreate");
